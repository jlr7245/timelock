...
import { getHMFrom5S, getHMLFrom5SDiff } from '../../../shared/utils';

const SiteListItem = ({ url, time, timeUsed }) => {
  const [hours, minutes] = getHMFrom5S(time);
  const [hRm, mRm] = getHMFrom5S(time - timeUsed);
  return (
    <div className="sli">
      ...
      <div className={`sli-time-remaining ${getHMLFrom5SDiff(time, timeUsed)}`}>
        <span className="sli-label">Time remaining</span>
        <p className="remaining vga">{hRm}:{mRm}</p>
      </div>
    </div>
  );
};
